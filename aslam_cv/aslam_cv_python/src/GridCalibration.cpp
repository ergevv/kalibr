...
typedef Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> corners_t;
...
boost::python::tuple findTarget3(aslam::cameras::GridDetector *gd,
                                 const aslam::Time &stamp,
                                 const corners_t &corners)  //不对称圆检测
{
  aslam::cameras::GridCalibrationTargetObservation obs(gd->target());
  // cv::Mat to;
  // eigen2cv(corners, to);
  bool success = gd->findTarget2(corners, stamp, obs);
