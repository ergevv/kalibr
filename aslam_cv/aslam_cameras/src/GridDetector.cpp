...
namespace aslam
{
  namespace cameras
  {
                                                  ...
                                                  GridCalibrationTargetObservation &outObservation) const
    {
      ...
      success = _target->computeObservation(image, cornerPoints, validCorners); // 找到角点cornerPoints，validCorners记录观测到的角点，因为有些可能脱离图像了
    ...
    bool GridDetector::findTarget2(const Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> &corners, const aslam::Time &stamp, GridCalibrationTargetObservation &outObservation) const // 这里的image是角点数据n*2
    {
      ...
      for (int i = 0; i < corners.rows(); i++)
      {
        ...
        outObservation.updateImagePoint(i, point);

        // outObservation.updateImagePoint(i, image.row(i).transpose());
      }
